...
def merge_k_fold_logs(k_fold_log_dir, process_pipeline):
    # ← CHANGE 1: only subdirectories, skip stray files
    fold_dirs = sorted([
        d for d in os.listdir(k_fold_log_dir)
        if os.path.isdir(os.path.join(k_fold_log_dir, d))
    ])
    k = len(fold_dirs)

    k_fold_metrics = {
        'acc':[],'bacc':[],'macro_auc':[],'micro_auc':[],'weighted_auc':[],
    ...
    for fold_dir in fold_dirs:
        ...
        # ← CHANGE 2: guard against missing csv
        matched = glob.glob(os.path.join(fold_log_dir, 'Best*.csv'))
        if not matched:
            print(f"⚠️  No Best*.csv in {fold_log_dir}, files: {os.listdir(fold_log_dir)}")
            continue
        
        best_log_df = pd.read_csv(matched[0])
        ...
        for i, key in enumerate(k_fold_metrics.keys()):
            k_fold_metrics[key].append(metrics.iloc[i])  # ← CHANGE 3: iloc for positional
    ...
    k_fold_metrics_mean_std = {
        ...
        for key, vals in k_fold_metrics.items()
    }

    merge_path = os.path.join(k_fold_log_dir, f'merge_{k}_fold_metrics.json')
    with open(merge_path, 'w') as f:
        json.dump(k_fold_metrics_mean_std, f, indent=4)
    print(f"✓ Merged {k} folds → {merge_path}")