...
class WSI_Dataset(Dataset):
    def __init__(self, dataset_info_csv_path, group, args): 
        self.dataset_info_csv_path = dataset_info_csv_path
        self.dataset_df = pd.read_csv(self.dataset_info_csv_path)   
        # using the existing train and test split with k-folds
        self.slide_path_list = self.dataset_df[f'{group}_slide_path'].dropna().to_list() 
        self.labels_list = self.dataset_df[f'{group}_label'].dropna().to_list()  
...
class WSI_Dataset_Test(Dataset):
    def __init__(self, dataset_info_csv_path, args, group, inference_mode=False): 
        self.dataset_info_csv_path = dataset_info_csv_path
        self.dataset_df = pd.read_csv(self.dataset_info_csv_path)
        self.inference_mode = inference_mode
        
        # Always get slide paths
        self.slide_path_list = self.dataset_df[f'{group}_slide_path'].dropna().to_list()
        
        # Get labels only if they exist
        if f'{group}_label' in self.dataset_df.columns:
            self.labels_list = self.dataset_df[f'{group}_label'].dropna().to_list()
        else:
            self.labels_list = []
    ...
    def __getitem__(self, idx):
        ...
        # Get label if available, otherwise use dummy label
        if self.labels_list:
            label = int(self.labels_list[idx])
            label = torch.tensor(label)
        else:
            label = torch.tensor(-1)  # Dummy label for inference
...
class SC_MIL_WSI_Dataset(WSI_Dataset):
    ...
    """
    def __init__(self, dataset_info_csv_path, h5_csv_path=None, group='train', args=None, use_dummy_coords=True):
        super(SC_MIL_WSI_Dataset, self).__init__(dataset_info_csv_path, group, args)
        self.use_dummy_coords = use_dummy_coords
        
        # Handle dict/None/string cases
        if isinstance(h5_csv_path, dict):
            h5_csv_path = None
        
        if h5_csv_path is not None and isinstance(h5_csv_path, str) and os.path.exists(h5_csv_path):
            ...
            if h5_csv_path is not None and isinstance(h5_csv_path, str):
