...
class WarmUpLR(_LRScheduler):
    ...
    def __init__(self, optimizer, warmup_epochs, base_lr):
        ...
        # We use base_lrs from the parent class which captures all param groups
        super().__init__(optimizer)

    def get_lr(self):
        """
        PyTorch calls this every step. 
        We return the LR for the CURRENT step (self.last_epoch).
        """
        if self.last_epoch < self.warmup_epochs:
            # Linear warmup: current_step / total_warmup_steps * base_lr
            # We do this for every parameter group in the optimizer
            return [base_lr * (self.last_epoch + 1) / self.warmup_epochs for base_lr in self.base_lrs]
        
        # If warmup is over, just return the base LRs
...
def get_criterion(criterion_name, alpha=None, gamma=2, reduction='mean'): 
    if criterion_name == 'ce':
        ...
        raise ValueError(f"Unknown criterion: {criterion_name}. Supported: 'ce', 'bce', 'focal_loss'")
...
def get_model_from_yaml(yaml_args):
    ...
    elif model_name == 'PTC_MIL':
        from modules.PTC_MIL.model_vit_pmt_clu import ViT_PMT_CLU 
        # Extract parameters with defaults
        top_k = yaml_args.Model.top_k if hasattr(yaml_args.Model, 'top_k') else 1
        n_layers = yaml_args.Model.n_layers if hasattr(yaml_args.Model, 'n_layers') else 2
        
        mil_model = ViT_PMT_CLU(
            n_classes=yaml_args.General.num_classes,
            args=yaml_args.Model,   # Passes the full model config node
            top_k=top_k,
            n_layers=n_layers
        )
        return mil_model
 
    elif model_name == 'DeepAttn_MIL':
        from modules.DeepAttn_MIL.deep_attn_mil import DeepAttnMIL_Surv 
        # Extract parameters with defaults
        in_dim = yaml_args.Model.in_dim if hasattr(yaml_args.Model, 'in_dim') else 1024
        embedding_dim = yaml_args.Model.embedding_dim if hasattr(yaml_args.Model, 'embedding_dim') else 64
        attention_dim = yaml_args.Model.attention_dim if hasattr(yaml_args.Model, 'attention_dim') else 32
        fc_dim = yaml_args.Model.fc_dim if hasattr(yaml_args.Model, 'fc_dim') else 32
        dropout = yaml_args.Model.dropout if hasattr(yaml_args.Model, 'dropout') else 0.5
        cluster_num = yaml_args.Model.cluster_num if hasattr(yaml_args.Model, 'cluster_num') else 10
        act = yaml_args.Model.act if hasattr(yaml_args.Model, 'act') else 'relu'
